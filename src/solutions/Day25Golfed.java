import java.util.*;
class Y{
Map m;List<List<Integer>>g;int N;
...
int c(int e){
var u=new int[N][N];
for(int f=0;f++<4;){int[]q=new int[N],P=new int[N];int h=0,t=1;P[0]=1;
while(h<t&P[e]<1){int v=q[h++];for(int n:g.get(v))if(P[n]<1&u[v][n]<1){P[n]=v+1;q[t++]=n;}}
if(P[e]<1)return f>3?t*(N-t):0;for(int x=e;x>0;x=P[x]-1){int y=P[x]-1;u[y][x]++;u[x][y]--;}}
return 0;
}
}
