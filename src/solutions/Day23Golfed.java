...
for(int v=L;v-->0;)if(h[v]>0)for(int d=4;d-->0;)if(a(G.charAt(v),d))for(int f=h[v]-1,x=v,t=d,s=0;;){if(!o(x+=D[t]))break;s++;if(h[x]>0){int n=h[x]-1;if(s>g[f][n])g[f][n]=s;break;}int n=4;for(;n-->0;)if(a(G.charAt(x),n)&o(x+D[n])&n!=(t^2))break;if(n<0)break;t=n;}
return""+d(h[S]-1);
}
