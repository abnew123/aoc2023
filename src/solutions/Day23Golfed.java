...
int s=0;for(;;){if(!o(v+=D[d]))return;s++;if(h[v]>0){var a=g[f];int i=0;while(a[i]>0)i++;a[i]=(h[v]-1)*L+s;return;}int n=4;for(;n-->0;)if(a(G.charAt(v),p,n)&o(v+D[n])&n!=(d^2))break;if(n<0)return;d=n;}
}
int d(int a,long s){if(a==e)return 0;int b=-L;for(int z:g[a])if(z>0&&(s>>z/L&1)<1)b=Math.max(b,z%L+d(z/L,s|1L<<z/L));return b;}
