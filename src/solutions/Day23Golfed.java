...
g=new int[N];
for(int v=L,n;v-->0;)if(h[v]>0)for(int d=4;d-->0;)if(a(G.charAt(v),d))for(int f=h[v]-5,x=v,t=d,s=0;;){if(!o(x+=D[t]))break;s++;if(h[x]>0){g[f+d]=h[x]/5*L-L+s;break;}n=4;while(n-->0&&!(a(G.charAt(x),n)&o(x+D[n])&n!=(t^2)));if(n<0)break;t=n;}
