...
return""+d(h[S]-1,1L<<h[S]-1);
}
void A(int[]h,int f,int v,int d,boolean p){
int s=0;for(;;){if(!o(v+=D[d]))return;s++;if(h[v]>0){var a=g[f];a[++a[0]*2-1]=h[v]-1;a[a[0]*2]=s;return;}int q=-1;for(int n=4;n-->0;)if(a(G.charAt(v),p,n)&o(v+D[n])&n!=(d^2))q=n;if(q<0)return;d=q;}
}
int d(int a,long s){if(a==e)return 0;int b=-1<<20;for(int i=1;i<g[a][0]*2;i+=2){int x=g[a][i];if((s>>x&1)<1)b=Math.max(b,g[a][i+1]+d(x,s|1L<<x));}return b;}
