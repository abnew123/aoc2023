...
void d(int z){boolean H=z<0;int x=z&1023,n=z>>10&1023,y=t[x];if(y<0)return;if(y>1){int q=p[x][0];for(;p[x][q]!=n;q--);if(H)M[x]|=1<<q;else M[x]&=~(1<<q);H=M[x]<F[x];}else if(y>0){if(H)return;H=f[x]=!f[x];}else H=0>1;for(int k:o[x])A(H,k,x);}
