...
long C(){int W=t[X]/8,s[]=new int[W+1];for(int P=1,F=0;P<10000&F<W;P++){e();while(u<v){int j=q[u]>>10&1023;if(q[u]<0&(q[u]&1023)==X&&s[j]<1){s[j]=P;F++;}d(q[u++]);}}long A=1;for(int j=W;j>0;)A*=s[j--];return A;}
