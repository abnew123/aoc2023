class X{
 ...
 boolean g(double[][]a){for(int i=0;i<6;i++){int p=i;for(int j=a.length;j-->i;)if(Math.abs(a[j][i])>Math.abs(a[p][i]))p=j;if(a[p][i]==0)return false;var q=a[p];a[p]=a[i];a[i]=q;for(int k=7;k-->i;)q[k]/=q[i];for(int j=a.length;j-->0;)if(j!=i)for(int k=7;k-->i;)a[j][k]-=a[j][i]*q[k];}return true;}
}
